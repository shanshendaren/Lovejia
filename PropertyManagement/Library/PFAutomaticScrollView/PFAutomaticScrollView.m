...
@interface NSTimer (PFAutomaticScrollView)
...
@implementation NSTimer (PFAutomaticScrollView)
...
//获取总页数
typedef NSInteger (^numberOfPagesBlock)(PFAutomaticScrollView *);

//获取内容页
typedef UIView *(^contentViewBlock)(PFAutomaticScrollView *, NSInteger);

//页控制器（白点）
typedef void (^pageControlBlock)(PFAutomaticScrollView *, UIPageControl *, NSInteger);

//文本
typedef void (^textLabelBlock)(PFAutomaticScrollView *, UILabel *, NSInteger);

//点击事件
typedef void (^tapBlock)(PFAutomaticScrollView *, NSInteger);

@interface PFAutomaticScrollView () <UIScrollViewDelegate>
{
...
@implementation PFAutomaticScrollView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //文本
        self.textLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.origin.x, self.bounds.size.height - 25, self.bounds.size.width, 30)];
        ...
        self.pageControl.frame =CGRectMake(self.bounds.size.width-80, 0, 80, 30);
        self.pageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:255.0/255.0 green:200.0/255.0 blue:0.0 alpha:1.0];
        self.pageControl.pageIndicatorTintColor = [UIColor whiteColor];
        self.pageControl.currentPage = 0;
        [self.textLabel addSubview:self.pageControl];
...
- (id)initWithFrame:(CGRect)frame animationDuration:(NSTimeInterval)animationDuration delegate:(id<PFAutomaticScrollViewDelegate>)delegate
{
    ...
    if (animationDuration > 0.0) {//设置计时器
        ...
        if (delegate) {//设置代理并获取页数
            ...
            [self setupTotalPagesCount:[self.delegate numberOfPagesInAutomaticScrollView:self]];
        } else {//暂停计时器
...
- (void)setScrollViewDataSource
{
    ...
    } else if ([self.delegate respondsToSelector:@selector(automaticScrollView:contentViewAtIndex:)]) {//监听代理并回调
        [contentViews addObject:[self.delegate automaticScrollView:self contentViewAtIndex:previousPageIndex]];
        [contentViews addObject:[self.delegate automaticScrollView:self contentViewAtIndex:currentPageIndex]];
        [contentViews addObject:[self.delegate automaticScrollView:self contentViewAtIndex:nextPageIndex]];
    }

    //设置页控制器（白点）
    if (!self.delegate && self.pageControlBlock) {//监听块并回调
        self.pageControlBlock(self, self.pageControl, currentPageIndex);
    } else if ([self.delegate respondsToSelector:@selector(automaticScrollView:pageControl:atIndex:)]) {//监听代理并回调
        [self.delegate automaticScrollView:self pageControl:self.pageControl atIndex:currentPageIndex];
    }

    //设置文本
    if (!self.delegate && self.textLabelBlock) {//监听块并回调
        self.textLabelBlock(self, self.textLabel, currentPageIndex);
    } else if ([self.delegate respondsToSelector:@selector(automaticScrollView:textLabel:atIndex:)]) {//监听代理并回调
        [self.delegate automaticScrollView:self textLabel:self.textLabel atIndex:currentPageIndex];
    }
}
...
//获取页数
- (void)numberOfPagesInAutomaticScrollViewUsingBlock:(NSInteger (^)(PFAutomaticScrollView *))block
{
...
//获取视图
- (void)contentViewAtIndexUsingBlock:(UIView *(^)(PFAutomaticScrollView *, NSInteger))block
{
...
//获取页控制器（白点）
- (void)pageControlAtIndexUsingBlock:(void (^)(PFAutomaticScrollView *, UIPageControl *, NSInteger))block
{
    if (block) {
...
//获取文本
- (void)textLabelAtIndexUsingBlock:(void (^)(PFAutomaticScrollView *, UILabel *, NSInteger))block
{
    if (block) {
...
//获取点击事件
- (void)didSelectItemAtIndexUsingBlock:(void (^)(PFAutomaticScrollView *, NSInteger))block
{
...
- (void)contentViewDidTap:(UITapGestureRecognizer *)recognizer
{
    ...
    } else if ([self.delegate respondsToSelector:@selector(automaticScrollView:didSelectItemAtIndex:)]) {//监听代理并回调
        [self.delegate automaticScrollView:self didSelectItemAtIndex:currentPageIndex];
    }
}
