...
//    [self.yearPicker selectRow:[self.yearArray count]-1 inComponent:0 animated:YES];
    ...
    [self.hourPicker selectRow:[[NSDate date] getHours] inComponent:0 animated:YES];
    ...
    NSString* str = [NSString stringWithFormat:@"%04d%02d%02d%02d%02d",self.yearValue, self.monthValue, self.dayValue, self.hourValue+8, self.minuteValue];
    [self setDate:[NSDate dateFromString:str withFormat:@"yyyyMMddHHmm"]];
}
#pragma mark -
-(void)createMonthArrayWithYear:(NSInteger)yearInt month:(NSInteger)monthInt{
