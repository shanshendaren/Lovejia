...
@interface FixInfoViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
{
...
-(void)createUI{
    ...
    //业务数据参数组织成JSON字符串
    NSString *biz = [NSString  stringWithFormat:@"{\"repairsId\":\"%@\"}",self.fixId];
    NSString *sid = @"FindRepairsById";
    ...
    contentView1= [[UITextView  alloc] initWithFrame:CGRectMake(85, 0, self.view.frame.size.width-90, 80)] ; //初始化大小
...
-(void)clickAction:(UIButton *)btn{
    if (isFeedback ==0) {
        RequestUtil *requestUtil = [[RequestUtil alloc]init];
        //业务数据参数组织成JSON字符串
        NSString *biz = [NSString  stringWithFormat:@"{\"repairsId\":\"%@\",\"feedbackStatus\":\"%d\"}",self.fixId,(int)btn.tag];
        NSString *sid = @"RepairsFeedback";
        [requestUtil startRequest:sid biz:biz send:self];
    }else{
...
-(void)requestCompleted:(ASIHTTPRequest *)request{
    ...
    if ([json[@"SID"]isEqualToString:@"FindRepairsById"]) {
        NSDictionary *dic = json[@"repairs"];
        Fix *fix = [[Fix alloc]init];
        if (dic) {
            fix.fixTitle =[dic objectForKey:@"repairsTitle"];
            fix.fixMess =[dic objectForKey:@"repairsDetail"];
            fix.fixTime =[dic objectForKey:@"repairsTime"];
            fix.fixAcceptRemark =[dic objectForKey:@"acceptRemark"];
            if ([dic objectForKey:@"isAccept"]) {
                fix.fix_isAccept =[[dic objectForKey:@"isAccept"] intValue];
            }
            if ([dic objectForKey:@"acceptTime"]) {
                fix.fixAcceptTime =[dic objectForKey:@"acceptTime"];
            }
            if ([dic objectForKey:@"feedbackStatus"]) {
                fix.fixFeedbackStatus =[dic objectForKey:@"feedbackStatus"];
            }
        }
        if ([json[@"status"]isEqualToString:@"success"]) {
            titleLabelInfo.text = [NSString stringWithFormat:@"%@",fix.fixTitle];
            timeLabel.text = [NSString stringWithFormat:@"%@",fix.fixTime];
            contentView.text = [NSString stringWithFormat:@"%@",fix.fixMess];
            contentView1.text = [NSString stringWithFormat:@"%@",fix.fixAcceptRemark];
            if (fix.fix_isAccept == 1) {
                isdone.text = [NSString stringWithFormat:@"报修已受理"];
            }else{
                isdone.text = [NSString stringWithFormat:@"报修还未受理"];
            }
            if (fix.fixFeedbackStatus) {
                int a =[fix.fixFeedbackStatus intValue];
                if (a ==3) {
    ...
    else if ([json[@"SID"]isEqualToString:@"RepairsFeedback"]){
        if ([json[@"status"]isEqualToString:@"success"]) {
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *string = [self.fix.photos objectAtIndex:indexPath.row][@"paths"];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    view.photoArray = self.fix.photos;
