...
- (void)headerRereshing
{
    ...
    //业务数据参数组织成JSON字符串
    NSString *biz = [NSString  stringWithFormat:@"{\"barterInfoId\":\"%@\",\"lastNum\":\"0\"}",self.valuablesInfo.barterInfoId];
    NSString *sid = @"QueryBarterCommonInfoList";
...
- (void)footerRereshing
{
    ...
    RequestUtil *requestUtil = [[RequestUtil alloc]init];
    //业务数据参数组织成JSON字符串
    NSString *biz = [NSString  stringWithFormat:@"{\"barterInfoId\":\"%@\",\"lastNum\":\"%@\"}",self.valuablesInfo.barterInfoId,dis.barterCommentInfoId];
    NSString *sid = @"QueryBarterCommonInfoList";
    [requestUtil startRequest:sid biz:biz send:self requestTag:1];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        // (最好在刷新表格后调用)调用endRefreshing可以结束刷新状态
...
-(void)requestCompleted:(ASIHTTPRequest *)request{
    ...
    NSArray *arr = [json objectForKey:@"barterCommonInfo"];
    if (isHead) {
        disArr = [[NSMutableArray alloc]init];
    }
    if ([json[@"status"]isEqualToString:@"success"]) {
        if (request.tag ==1) {
            for (int i = 0; i <arr.count ; i++) {
                ...
                dis.barterCommentInfoId = [NSString stringWithFormat:@"%@",[rdic objectForKey:@"barterCommentInfoId"]];
                dis.barterInfoId = [NSString stringWithFormat:@"%@",[rdic objectForKey:@"barterInfoId"]];
                dis.barterCommentContent = [NSString stringWithFormat:@"%@",[rdic objectForKey:@"barterCommentContent"]];
...
-(void)disagreeAction:(UIButton *)sender{
    if (disArr.count >0) {
        ...
        if ([dis.isAgree isEqualToString:@"1"]||[dis.isAgree isEqualToString:@"0"]) {
            ...
            RequestUtil *requestUtil = [[RequestUtil alloc]init];
            //业务数据参数组织成JSON字符串
            NSString *biz = [NSString  stringWithFormat:@"{\"barterCommonInfoId\":\"%@\",\"dealWithReslut\":\"0\",\"registrationId\":\"%@\",\"deviceType\":\"%@\"}",dis.barterCommentInfoId,self.valuablesInfo.registrationId,self.valuablesInfo.deviceType];
            NSString *sid = @"DealWithBarterCommonInfo";
...
-(void)agreeAction:(UIButton *)sender{
    if (disArr.count >0) {
        ...
        if ([dis.isAgree isEqualToString:@"1"]||[dis.isAgree isEqualToString:@"0"]) {
            ...
            RequestUtil *requestUtil = [[RequestUtil alloc]init];
            //业务数据参数组织成JSON字符串
            NSString *biz = [NSString  stringWithFormat:@"{\"barterCommonInfoId\":\"%@\",\"dealWithReslut\":\"1\",\"registrationId\":\"%@\",\"deviceType\":\"%@\"}",dis.barterCommentInfoId,self.valuablesInfo.registrationId,self.valuablesInfo.deviceType];
            NSString *sid = @"DealWithBarterCommonInfo";
