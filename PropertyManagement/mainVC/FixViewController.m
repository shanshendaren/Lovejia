...
@interface FixViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIActionSheetDelegate>{
    BOOL isOpened;
    int fixType;
    UITextField *fixTitle;
...
-(void)cilick{
    ...
    if (fixTitle.text.length == 0) {
        [self doAlert:@"请输入标题"];
        [fixTitle becomeFirstResponder];
        return;
    ...
    else if (fixTitle.text.length > 25){
        [self doAlert:@"标题过长"];
        [fixTitle becomeFirstResponder];
        return;
    }
    ...
    //业务数据参数组织成JSON字符串
    NSString *biz = [NSString  stringWithFormat:@"{\"userId\":\"%@\",\"title\":\"%@\",\"content\":\"%@\",\"type\":\"%d\"}",app.userID,fixTitle.text,contentView.text,fixType];
    NSString *sid = @"DoRepairs";
...
//{
//    if ([contentView.text isEqualToString:@"请输入内容"]) {
//        contentView.text=@"";
//    }
//    contentView.textColor = [UIColor blackColor];
//    return YES;
