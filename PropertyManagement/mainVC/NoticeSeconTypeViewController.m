//
//  NoticeSeconTypeViewController.m
//  PropertyManagement
//
...
@interface NoticeSeconTypeViewController (){
...
@implementation NoticeSeconTypeViewController
...
- (void)headerRereshing
{
    ...
    //业务数据参数组织成JSON字符串
    NSString *biz = [NSString  stringWithFormat:@"{\"userId\":\"%@\",\"lastNum\":\"0\",\"type\":\"99\"}",app.userID];
    NSString *sid = @"FindMessageList";
...
- (void)footerRereshing
{
    BZAppDelegate *app = [UIApplication sharedApplication].delegate;
    isHead = NO;
    Notice * notice = [noticeArr lastObject];
    RequestUtil *requestUtil = [[RequestUtil alloc]init];
    if (notice) {
        //业务数据参数组织成JSON字符串
        NSString *biz = [NSString  stringWithFormat:@"{\"userId\":\"%@\",\"lastNum\":\"%@\",\"type\":\"99\"}",app.userID,notice.messId];
        NSString *sid = @"FindMessageList";
        [requestUtil startRequest:sid biz:biz send:self];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            // (最好在刷新表格后调用)调用endRefreshing可以结束刷新状态
...
-(void)requestCompleted:(ASIHTTPRequest *)request{
    ...
    NSArray *arr = [json objectForKey:@"mess"];
    if (isHead) {
        noticeArr = [[NSMutableArray alloc]init];
    }
    if ([json[@"status"]isEqualToString:@"success"]) {
        for (int i = 0; i <arr.count ; i++) {
            ...
            notice.messId = [rdic objectForKey:@"messId"];
            ...
            notice.messDetail = [rdic objectForKey:@"messContent"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    Notice * notice = [noticeArr objectAtIndex:indexPath.row];
    newcell.timeLabel.text = [NSString stringWithFormat:@"%@",notice.messTime];
    newcell.nameLabel.text = [NSString stringWithFormat:@"%@",notice.messTitle];
    newcell.infoLabel.text =[NSString stringWithFormat:@"%@",notice.messDetail];
    return newcell;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    Notice *notice = [noticeArr objectAtIndex:indexPath.row];
    NoticeInfoViewController *nic = [[NoticeInfoViewController alloc]init];
    nic.noticeId = notice.messId;
