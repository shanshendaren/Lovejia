...
#import "BZFaLvSetViewController.h"
...
@interface BZFaLvListViewController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation BZFaLvListViewController
...
- (void)headerRereshing
{
    ...
    //业务数据参数组织成JSON字符串
    NSString *biz = [NSString  stringWithFormat:@"{\"vallageId\":\"%@\",\"userId\":\"%@\",\"lastNum\":\"0\"}",app.vallageID,app.userID];
    NSString *sid = @"LegalAidlist";
...
- (void)footerRereshing
{
    ...
    if (barter) {
        //业务数据参数组织成JSON字符串
        NSString *biz = [NSString  stringWithFormat:@"{\"vallageId\":\"%@\",\"userId\":\"%@\",\"lastNum\":\"%@\"}",app.vallageID,app.userID,barter.barterInfoId];
        NSString *sid = @"LegalAidlist";
        [requestUtil startRequest:sid biz:biz send:self];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            // (最好在刷新表格后调用)调用endRefreshing可以结束刷新状态
...
-(void)requestCompleted:(ASIHTTPRequest *)request{
    ...
    NSLog(@">>date>>>>%@",returnData);
    NSArray *arr = [json objectForKey:@"type"];
    if (isHead) {
        barterArr = [[NSMutableArray alloc]init];
    }
    if ([json[@"status"]isEqualToString:@"success"]) {
        if ([json[@"isComplete"]isEqualToString:@"yes"]) {
            ...
            for (int i = 0; i <arr.count ; i++) {
                ...
                barter.barterInfoId = [rdic objectForKey:@"id"];
                barter.barterInfoTitle = [NSString stringWithFormat:@"%@",[rdic objectForKey:@"title"]];
                barter.barterInfoContent = [NSString stringWithFormat:@"%@",[rdic objectForKey:@"content"]];
                barter.repaeaseDate =[NSString stringWithFormat:@"%@",rdic[@"repease_date"]];
                barter.repaeaseName = [NSString stringWithFormat:@"%@",rdic[@"owner_name"]];
                barter.repaeaseTel = [NSString stringWithFormat:@"%@",rdic[@"phone"]];
...
-(void)pushAction{
    BZFaLvSetViewController *svc =[[BZFaLvSetViewController alloc]init];
