...
-(void)createUI{
    ...
    isHead = NO;
    setTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width,  self.view.bounds.size.height-[VersionAdapter getMoreVarHead]-44-40)];
    setTable.delegate = self; setTable.dataSource = self;
    [self.view addSubview:setTable];
    
    activity = [[ActivityView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - 44 -40- [VersionAdapter getMoreVarHead]) loadStr:NSLocalizedString(@"正在加载...", nil)];
...
- (void)headerRereshing
{
    ...
    //业务数据参数组织成JSON字符串
    NSString *biz = [NSString  stringWithFormat:@"{\"vallageId\":\"%@\",\"lastNum\":\"0\",\"saleType\":\"0\"}",app.vallageID];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    House *house = [houseArr objectAtIndex:indexPath.row];
    SaleHouseInfoViewController *nic = [[SaleHouseInfoViewController alloc]init];
    nic.house = house;
