...
@interface BZFaLvSetViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIActionSheetDelegate>{
    BOOL isOpened;
    NSString *helpType;
...
@implementation BZFaLvSetViewController
...
-(void)createUI{
    ...
    [_tb initTableViewDataSourceAndDelegate:^(UITableView *tableView,NSInteger section){
        ...
        helpType = [NSString stringWithFormat:@"%@",[[self.listTeams objectAtIndex:indexPath.row]objectForKey:@"st_id"]];
    ...
    [self.view addSubview:back_four];
...
-(void)cilick{
    ...
    //业务数据参数组织成JSON字符串
    NSString *biz = [NSString  stringWithFormat:@"{\"userId\":\"%@\",\"title\":\"%@\",\"content\":\"%@\",\"type\":\"%@\",\"isOpen\":\"%d\",\"vallageId\":\"%@\",\"phone\":\"%@\"}",app.userID,complainTitle.text ,contentView.text,helpType,mobileType,app.vallageID,app.userMobile];
    NSString *sid = @"SaveLegalAid";
