...
- (void)headerRereshing
{
    ...
    //业务数据参数组织成JSON字符串
    NSString *biz = [NSString  stringWithFormat:@"{\"userId\":\"%@\",\"lastNum\":\"0\",\"type\":\"2\"}",app.userID];
...
- (void)footerRereshing
{
    ...
    //业务数据参数组织成JSON字符串
    if (notice) {
        NSString *biz = [NSString  stringWithFormat:@"{\"userId\":\"%@\",\"lastNum\":\"%@\",\"type\":\"2\"}",app.userID,notice.messId];
        NSString *sid = @"FindMessageList";
        [requestUtil startRequest:sid biz:biz send:self];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            // (最好在刷新表格后调用)调用endRefreshing可以结束刷新状态
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    newcell.nameLabel.text = [NSString stringWithFormat:@"%@:%@",notice.messType,notice.messTitle];
